...
var AnnotationSelector = Panel.extend({
    ...
    initialize(settings = {}) {
        ...
        this.listenTo(
            this.collection,
            "sync remove update reset change:displayed change:loading",
            this._debounceRender
        );
        this.listenTo(
            this.collection,
            "change:highlight",
            this._changeAnnotationHighlight
        );
        this.listenTo(
            eventStream,
            "g:event.job_status",
            _.debounce(this._onJobUpdate, 500)
        );
        this.listenTo(
            eventStream,
            "g:eventStream.start",
        ...
        this.listenTo(
            eventStream,
            "g:event.large_image_annotation.create",
            this._refreshAnnotations
        );
        this.listenTo(
            eventStream,
            "g:event.large_image_annotation.remove",
            this._refreshAnnotations
        );
        this.listenTo(
            this.collection,
            "change:annotation change:groups",
    ...
    _settingsFromConfig() {
        if (
            ...
            !this.parentView._folderConfig.settings
        ) {
            return;
        }
        ...
        if (
            ...
            settings.annotationLabels !== undefined
        ) {
        ...
        if (
            this._interactiveMode === undefined &&
            settings.interactiveHover !== undefined
        ) {
            if (
                this._interactiveMode !==
                (settings.interactiveHover === true)
            ) {
        ...
        if (this.parentItem && this.parentItem.get("folderId")) {
            ...
            this.$el.html(
                annotationSelectorWidget({
                    ...
                    // title: 'Annotations',
                    title: "标签",
                    activeAnnotation: this._activeAnnotation
                        ? this._activeAnnotation.id
                        : "",
    ...
    _debounceRender() {
        if (!this._debounceRenderRequest) {
            this._debounceRenderRequest = window.requestAnimationFrame(() => {
                this.render();
            });
        }
        return this;
    ...
    setItem(item) {
        ...
        this.collection.fetch({ itemId: this._parentId }).then(() => {
            ...
            this.collection.each((model) => {
                if (
                    ((model.get("annotation") || {}).display || {}).visible ===
                    true
                ) {
    ...
    deleteAnnotation(evt) {
        ...
        if (model) {
            const name =
                (model.get("annotation") || {}).name || "unnamed annotation";
    ...
    _setCreationAccess(root, folderId) {
        ...
        this._setCreationRequest
            .done((createResp) => {
                root.creationAccess = createResp;
                root.$(".h-create-annotation").toggleClass(
                    "hidden",
                    !createResp
                );
                if (
                    this.parentItem &&
                    this.parentItem.get("folderId") === folderId
                ) {
            ...
            this.collection.each((model) => {
                if (!_.has(models, model.id)) {
                    ...
                    if (!refreshed) {
                        ...
                        // (above)
                        model.attributes.displayed =
                            models[model.id].get("displayed");
                    }
                }
            });
            this._debounceRender();
    ...
    editAnnotation(model) {
        ...
        if (
            ...
            model &&
            this._activeAnnotation.id === model.id
        ) {
            this._activeAnnotation = null;
    ...
    _setActiveAnnotationWithoutLoad(model) {
        const numElements = ((model.get("annotation") || {}).elements || [])
            .length;
    ...
    _saveAnnotation(annotation) {
        ...
        if (
            ...
            if (
                annotation
                    .elements()
                    .models.filter((model) => model.get("type") === "pixelmap")
                    .length === 0
            ) {
            ...
            annotation
                ...
                .fail(() => {
                     ...
                     * of retries. */
                    annotation._saveAgain = Math.min(
                        lastSaveAgain ? lastSaveAgain * 2 : 5,
                        300
                    );
                })
                .always(() => {
                    ...
                    if (
                        annotation._saveAgain !== undefined &&
                        annotation._saveAgain !== false
                    ) {
                    ...
                    if (
                        !Object.keys(vsaving).length ||
                        (Object.keys(vsaving).length === 1 && vsaving.refresh)
                    ) {
    ...
    _changeAnnotationHighlight(model) {
        this.$(`.h-annotation[data-id="${model.id}"]`).toggleClass(
            "h-highlight-annotation",
    ...
    _changeGlobalOpacity() {
        ...
        this.$(".h-annotation-opacity-container").attr(
            "title",
            `Annotation total opacity ${(this._opacity * 100).toFixed()}%`
        );
    ...
    _changeGlobalFillOpacity() {
        ...
        this.$(".h-annotation-fill-opacity-container").attr(
            "title",
            `Annotation fill opacity ${(this._fillOpacity * 100).toFixed()}%`
        );
    ...
    _getAnnotationGroups() {
        ...
        _.each(groups, (group) => {
            const groupList = this.collection.filter((a) =>
                _.contains(a.get("groups"), group)
            );
    ...
    _triggerGroupCountChange(groups) {
        ...
        _.each(groups, (annotations, name) => {
            if (name !== "Other") {
                ...
                groupCount[this.parentView._defaultGroup || "default"] =
                    annotations.length;
            }
        });
