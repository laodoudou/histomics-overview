...
var DrawWidget = Panel.extend({
    events: _.extend(Panel.prototype.events, {
        ...
        "click .h-group-count-option .h-group-count-select":
            "selectElementsInGroup",
        "change .h-style-group": "_setToSelectedStyleGroup",
        "change .h-brush-shape,.h-brush-size,.h-brush-screen": "_changeBrush",
        "change .h-fixed-shape,.h-fixed-height,.h-fixed-width":
            "_changeShapeConstraint",
    ...
    initialize(settings) {
        ...
        this.listenTo(
            this._groups,
            "add change",
            this._handleStyleGroupsUpdate
        );
        this.listenTo(this._groups, "remove", this.render);
        this.listenTo(
            this.collection,
            "add remove reset",
            this._recalculateGroupAggregation
        );
        ...
        this._groups.fetch().done(() => {
            ...
            if (this._groups.has(this.parentView._defaultGroup)) {
                this._style.set(
                    this._groups.get(this.parentView._defaultGroup).toJSON()
                );
            } else {
            ...
            if (
                this._editOptions.style &&
                this._groups.get(this._editOptions.style)
            ) {
                this._setStyleGroup(
                    this._groups.get(this._editOptions.style).toJSON()
                );
            }
        });
        this.on("h:mouseon", (model) => {
            if (model && model.id) {
                ...
                this.$(`.h-element[data-id="${model.id}"]`).addClass(
                    "h-highlight-element"
                );
            }
        });
        this.on("h:mouseoff", (model) => {
            if (model && model.id) {
                ...
                this.$(`.h-element[data-id="${model.id}"]`).removeClass(
                    "h-highlight-element"
                );
            }
        });
    ...
    render(updatedElement) {
        ...
        const name =
            (this.annotation.get("annotation") || {}).name || "Untitled";
        if (
            !updatedElement ||
            (updatedElement.attributes &&
                updatedElement.get("type") !== "pixelmap")
        ) {
        ...
        if (this.$(".h-group-count-option").length > 0) {
            this.$(".h-group-count-options").append(
                this.$(".h-group-count-option")
            );
        } else {
        ...
        if (this._drawingType) {
            this.$(
                'button.h-draw[data-type="' + this._drawingType + '"]'
            ).addClass("active");
    ...
    _bindHUIModeChange() {
        if (
            this.viewer.annotationLayer &&
            this.viewer.annotationLayer._boundHUIModeChange !== this
        ) {
            this.viewer.annotationLayer._boundHUIModeChange = this;
            this.viewer.annotationLayer.geoOff(geo.event.annotation.mode);
            this.viewer.annotationLayer.geoOn(
                geo.event.annotation.mode,
                (event) => {
                    if (
                        event.mode === this.viewer.annotationLayer.modes.edit ||
                        event.oldmode === this.viewer.annotationLayer.modes.edit
                    ) {
                        return;
                    }
        ...
        this.listenToOnce(dialog, "h:editElement", (obj) => {
            if (obj.edited) {
                ...
                const labelElement = this.$(
                    `.h-element[data-id="${id}"] .h-element-label`
                );
                ...
                if (label) {
                    ...
                    newLabel = `${
                        group || this.parentView._defaultGroup
                    } ${elemType} ${oldnum}`;
                } else {
                    newLabel = oldLabel;
                }
                this.$(`.h-element[data-id="${id}"] .h-element-label`)
                    .text(newLabel)
                    .attr("title", label);
                if (
                    origGroup !== group &&
                    ...
                    this.updateCount(
                        origGroup || this.parentView._defaultGroup,
                        -1
                    );
    ...
    viewElement(evt) {
        const annot =
            this.collection._byId[$(evt.target).parent().attr("data-id")];
        let points;
        ...
        if (
            Math.abs(newView.zoom - 1.5 - map.zoom()) <= 0.5 &&
            map.zoom() < newView.zoom
        ) {
            newView.zoom = false;
        }
        const distance =
            ((newView.center.x - map.center().x) ** 2 +
                (newView.center.y - map.center().y) ** 2) **
            0.5;
        map.transition({
    ...
    deleteElement(evt, id, opts) {
        ...
        if (
            ["point", "polyline", "rectangle", "ellipse", "circle"].includes(
                this.collection.get(id).attributes.type
            )
        ) {
            this.updateCount(
                this.collection.get(id).attributes.group ||
        ...
        this.newElementDisplayIdStart = +(
            this.$el
                .find(".h-element>span.h-element-label[display_id]")
                .last()
                .attr("display_id") || 0
        );
    },
    ...
    _applyBooleanOp(annotations, evtOpts) {
        if (
            !evtOpts.asPolygonList &&
            (annotations.length !== 1 || !annotations[0].toPolygonList)
        ) {
            return false;
        }
        const op = evtOpts.currentBooleanOperation;
        const existing = this.viewer._annotations[
            this.annotation.id
        ].features.filter(
            (f) => ["polygon", "marker"].indexOf(f.featureType) >= 0
        );
        const polylist = evtOpts.asPolygonList
            ? annotations
            : annotations[0].toPolygonList({
        ...
        polylist.forEach((poly) =>
            poly[0].forEach((pt) => searchPoly.push({ x: pt[0], y: pt[1] }))
        );
        const near = existing.map((f) =>
            f.polygonSearch(searchPoly, { partial: true }, null)
        );
        ...
        near.forEach((n) =>
            n.found.forEach((element) => {
                // filter to match current style group
                if (
                    element.properties.element &&
                    element.properties.element.group !==
                        this._style.get("group")
                ) {
                    return;
                }
        ...
        Object.keys(oldids).forEach((id) =>
            this.deleteElement(undefined, id, { silent: elements.length })
        );
        this.addElements(
            _.map(elements, (el) => {
    ...
    _brushPan() {
        ...
        if (zoom !== this._brushZoom) {
            ...
            this._setBrushCoordinates(
                this.viewer.annotationLayer.annotations()[0],
                size
            );
    ...
    _setBrushCoordinates(annot, size) {
        const center = this.viewer.viewer.interactor().mouse().mapgcs || {
            x: 0,
            y: 0,
        };
        annot._coordinates([
    ...
    _brushAction(evt) {
        ...
        let elements = [
            convertAnnotation(this.viewer.annotationLayer.annotations()[0]),
        ];
        if (!elements[0].id) {
        ...
        if (evt.event === geo.event.annotation.cursor_action) {
            ...
            if (
                ...
                this._lastBrushState.stateId &&
                this._lastBrushState.stateId === evt.evt.state.stateId
            ) {
                ...
                const bbox1 = this.viewer.annotationLayer
                    .annotations()[0]
                    ._coordinates();
                ...
                if (bbox1[0].x !== bbox2[0].x || bbox1[0].y !== bbox2[0].y) {
                    ...
                    if (shape === "square") {
                        ...
                        const ang =
                            Math.atan2(c2y - c1y, c2x - c1x) + Math.PI / 2;
                        blur = [
                            [
                                [
                                    [
                                        c1x + (size / 2) * Math.cos(ang),
                                        c1y + (size / 2) * Math.sin(ang),
                                    ],
                                    [
                                        c1x - (size / 2) * Math.cos(ang),
                                        c1y - (size / 2) * Math.sin(ang),
                                    ...
                                    [
                                        c2x - (size / 2) * Math.cos(ang),
                                        c2y - (size / 2) * Math.sin(ang),
                                    ],
                                    [
                                        c2x + (size / 2) * Math.cos(ang),
                                        c2y + (size / 2) * Math.sin(ang),
            ...
            this._lastBrushState.bbox = this.viewer.annotationLayer
                .annotations()[0]
                ._coordinates();
        } else {
    ...
    _setBrushMode(forceRefresh) {
        ...
        const annot = geo.registries.annotations[
            shape === "square" ? "rectangle" : shape
        ].func({ layer: this.viewer.annotationLayer });
        this.viewer.annotationLayer.addAnnotation(annot);
        this._setBrushCoordinates(annot, size);
        this.viewer.annotationLayer.mode(
            this.viewer.annotationLayer.modes.cursor,
            annot
        );
    ...
    _addDrawnElements(element, annotations, opts) {
        ...
        if (opts.currentBooleanOperation) {
            ...
            if (
                processed ||
                ["difference", "intersect"].indexOf(
                    opts.currentBooleanOperation
                ) >= 0
            ) {
                this.drawElement(undefined, this._drawingType);
                return undefined;
    ...
    drawElement(evt, type, forceRefresh) {
        ...
        if (
            this.viewer.annotationLayer.mode() === type &&
            this._drawingType === type &&
            (!type || this.viewer.annotationLayer.currentAnnotation) &&
            !forceRefresh
        ) {
            return;
        }
        if (this.viewer.annotationLayer.mode()) {
            ...
            this.viewer.annotationLayer.geoOff(geo.event.annotation.state);
            if (this._brushPanBound) {
                this.viewer.annotationLayer.geoOff(
                    geo.event.pan,
                    this._brushPanBound
                );
            }
        ...
        if (type === "brush") {
            ...
            if (this._editOptions.size_mode === "fixed_aspect_ratio") {
                options.modeOptions.constraint =
                    this._editOptions.fixed_width /
                    this._editOptions.fixed_height;
            ...
            this.viewer
                .startDrawMode(type, options)
                .then((element, annotations, opts) =>
                    this._addDrawnElements(element, annotations, opts)
                )
                .fail(() => {
                    if (
                        this._drawingType &&
                        this._drawingType !==
                            this.viewer.annotationLayer.mode() &&
                        this.viewer.annotationLayer.mode() !== "edit"
                    ) {
                        this.drawElement(
                            undefined,
                            this._drawingType,
    ...
    _getEditOptions() {
        ...
        try {
            hui = JSON.parse(
                window.localStorage.getItem("histomicsui") || "{}"
            );
        } catch (err) {}
        if (!_.isObject(hui)) {
            hui = {};
    ...
    _verifyEditOptions(opts, raiseOnError) {
        if (
            raiseOnError &&
            opts.brush_shape &&
            ["square", "circle"].indexOf(opts.brush_shape) < 0
        ) {
    ...
    updateCount(groupName, change) {
        const groupElem = $(
            '.h-group-count-options > [data-group="' + groupName + '"]'
        );
        if (groupElem.length > 0) {
            ...
            if (newCount > 0) {
                for (const group of $(".h-group-count-option").toArray()) {
                    ...
                    if (newCount > count) {
                        ...
                        group === $(".h-group-count-options:last-child")[0]
                    ) {
                        $(group).after(groupElem);
                    }
                }
                groupElem
                    .find(".h-group-count-value")
                    .html(newCount + " " + groupName)
                    .show();
            } else {
                groupElem.remove();
            }
            ...
            const span =
                (
                    $(
                        ".h-group-count-option:first span.h-group-count-select"
                    )[0] || {}
                ).outerHTML || "";
            const newrecord =
                '<div class="h-group-count-option" data-group="' +
                groupName +
                '" data-count=' +
                change +
                '><span class="h-group-count-value">' +
                change +
                " " +
                groupName +
                "</span>" +
                span +
                "</div>";
            for (const group of $(".h-group-count-option")
                .toArray()
                .reverse()) {
                if (
                    $(group).attr("data-count") > change ||
                    ($(group).attr("data-count") === change &&
                        $(group).attr("data-group") < groupName)
                ) {
                    $(group).after(newrecord);
                    break;
                } else if (
                    group === $(".h-group-count-options:first-child")[0]
                ) {
    ...
    countPixelmap(pixelmap, operation) {
        ...
        for (let ix = 0; ix < pixelmap.get("values").length; ix++) {
            const groupName =
                pixelmap.get("categories")[pixelmap.get("values")[ix]].label ||
                this.parentView._defaultGroup;
    ...
    _setStyleGroup(group) {
        ...
        if (!group.group && this._style.id !== this.parentView._defaultGroup) {
            ...
            this._style.get("group") &&
            this._style.id === this.parentView._defaultGroup
        ) {
    ...
    _dropdownControlClick(e) {
        ...
        $(e.target)
            .find(".icon-down-open,.icon-up-open")
            .removeClass(isCollapsed ? "icon-down-open" : "icon-up-open")
            .addClass(isCollapsed ? "icon-up-open" : "icon-down-open");
        // Select the corresponding radio button for the current size_mode
        $(
            `input[mode="${this._editOptions.size_mode || "unconstrained"}"]`,
            $(e.target.parentNode)
        ).trigger("click");
    },
    ...
    _changeBrush(e) {
        ...
        this.$('.h-draw[data-type="brush"]').attr(
            "shape",
            this._editOptions.brush_shape
        );
    ...
    _updateConstraintValueInputs() {
        if (
            ["fixed_aspect_ratio", "fixed_size"].includes(
                this.$(".h-fixed-shape:checked").attr("mode")
            )
        ) {
    ...
    nextBrushShape() {
        this.$(
            '.h-brush-shape[name="h-brush-shape"][shape="' +
                this.$('.h-brush-shape[name="h-brush-shape"]:checked').attr(
                    "next_shape"
                ) +
                '"]'
        ).prop("checked", true);
    ...
    adjustBrushSize(delta) {
        const newval = Math.max(
            1,
            parseFloat(this.$(".h-brush-size").val()) + delta
        );
    ...
    _highlightElement(evt) {
        ...
        if (
            this.annotationSelector._interactiveMode &&
            ["point", "polyline", "rectangle", "ellipse", "circle"].includes(
                annotType
            )
        ) {
