...
wrap(ItemView, "render", function (render) {
    ...
    this.once("g:rendered", function () {
        HuiSettings.getSettings().then((settings) => {
            ...
            if (
                this.$el.find('.g-edit-item[role="menuitem"]').length &&
                !this.$el.find('.g-hui-quarantine-item[role="menuitem"]').length
            ) {
                this.$el
                    .find('.g-edit-item[role="menuitem"]')
                    .parent("li")
                    .after(
            ...
            if (
                !this.$el.find('.g-hui-open-item[role="button"]').length &&
                ...
                this.$el.find(".g-item-header .btn-group").before(
                    `<a class="g-hui-open-item btn btn-sm btn-primary" role="button" href="${webrootPath}#?image=${this.model.id}" target="_blank">
                            <i class="icon-link-ext"></i>${brandName}
                    </a>`
                );
            }
